...
#endif
static unsigned long long elapsedTime;
#if defined(MSVC_COMPILER)
...
#define NULL_RET 0
#if M_INTERFACE==1
DWORD WINAPI beastTrend_demo(__in LPVOID lpParameter)
{
...
DWORD WINAPI beastTrend_demo(__in LPVOID lpParameter)
{
	...
	MEM.init(&MEM);
	Options		opt;
	get_options_trend(&opt,pY,pOpt,&MEM);
	if (!opt.outputToDisk && nlhs==0) {
		print_error(1,&MEM);
		return NULL_RET;
	}
	print_options_trend(&opt);
	if (gData.N <=0)
	{
	...
	int32_t * _restrict rowsPerStrip=NULL;
	int32_t * _restrict startIdxOfStrip=NULL;
	...
	if (opt.computeCredible)
	{
		...
		rowsPerStrip=MEM.alloc(&MEM,sizeof(int)*numStrips,0);
		startIdxOfStrip=MEM.alloc(&MEM,sizeof(int)*numStrips,0);
		uint32_t stripTmpNum=0;
		for (uint32_t i=1; i <=numStrips; i++)
		{
	...
	{
		BASISTEMP1.XtX=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K*MAX_K,64);
		BASISTEMP2.XtX=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K*MAX_K,64);
		BASISTEMP1.XtY=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K,64);
		BASISTEMP2.XtY=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K,64);
		BASISTEMP1.post_P_U=(float *)MEM.alloc(&MEM,sizeof(float)*MAX_K* MAX_K,64);
		BASISTEMP2.post_P_U=(float *)MEM.alloc(&MEM,sizeof(float)*MAX_K* MAX_K,64);
		BASISTEMP1.beta_mean=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K,64);
		BASISTEMP2.beta_mean=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K,64);
		BASISTEMP1.beta=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K,64);
		BASISTEMP2.beta=(float *)MEM.alloc(&MEM,sizeof(float)* MAX_K,64);
		if ((opt.maxKnotNum_Trend+2) * 2 * sizeof(uint16_t) <=64)
		{
			BASISTEMP1.T=(uint16_t *)MEM.alloc(&MEM,64,64);
			*BASISTEMP1.T++=1L;
		...
		{
			BASISTEMP1.T=(uint16_t *)MEM.alloc(&MEM,sizeof(uint16_t)* (opt.maxKnotNum_Trend+2),64);
			*BASISTEMP1.T++=1L;
			BASISTEMP2.T=(uint16_t *)MEM.alloc(&MEM,sizeof(uint16_t)* (opt.maxKnotNum_Trend+2),64);
			*BASISTEMP2.T++=1L;
		}
		if ((opt.maxKnotNum_Trend+1) * 2 * sizeof(char) <=64)
		{
			BASISTEMP1.tOrder=(uint8_t *)MEM.alloc(&MEM,64,64);
			BASISTEMP2.tOrder=(uint8_t *)BASISTEMP1.tOrder+(opt.maxKnotNum_Trend+1);
		}
		else
		{
			BASISTEMP1.tOrder=(uint8_t *)MEM.alloc(&MEM,sizeof(char)*(opt.maxKnotNum_Trend+1),64);
			BASISTEMP2.tOrder=(uint8_t *)MEM.alloc(&MEM,sizeof(char)*(opt.maxKnotNum_Trend+1),64);
		}
		BASISTEMP1.tks=(int16_t *)MEM.alloc(&MEM,sizeof(int16_t)* MAX_K,64);
		BASISTEMP2.tks=(int16_t *)MEM.alloc(&MEM,sizeof(int16_t)* MAX_K,64);
		BASISTEMP1.tke=(int16_t *)MEM.alloc(&MEM,sizeof(int16_t)* MAX_K,64);
		BASISTEMP2.tke=(int16_t *)MEM.alloc(&MEM,sizeof(int16_t)* MAX_K,64);
		BASISTEMP1.termType=(uint8_t *)MEM.alloc(&MEM,sizeof(char)* MAX_K,64);
		BASISTEMP2.termType=(uint8_t *)MEM.alloc(&MEM,sizeof(char)* MAX_K,64);
	}
	F32PTR Xt_mars=(float *)MEM.alloc(&MEM,sizeof(float)* opt.Npad*MAX_K,64);
	F32PTR Xnewterm=(float *)MEM.alloc(&MEM,sizeof(float)* opt.Npad*MAX_K,64);
	F32PTR Xt_zeroBackup=(float *)MEM.alloc(&MEM,sizeof(float)* opt.Npad*MAX_K,64);
	U08PTR goodT;
	uint32_t goodT_num;
	goodT=(uint8_t *) MEM.alloc(&MEM,sizeof(char)*opt.Npad16,64);	
	...
	TREND_TERMS=(float*)MEM.alloc(&MEM,opt.N*sizeof(float)* (opt.maxTrendOrder+1),64);
	INV_SQR=(float*)MEM.alloc(&MEM,opt.N*sizeof(float),0);
	COEFF_A=(float*)MEM.alloc(&MEM,opt.N*sizeof(float),0);
	COEFF_B=(float*)MEM.alloc(&MEM,opt.N*sizeof(float),0);
	preCompute_Xmars_terms_fast(SEASON_TERMS,SEASON_SQR_CSUM,TREND_TERMS,INV_SQR,COEFF_A,COEFF_B,opt.N,opt.period,opt.maxSeasonOrder,opt.maxTrendOrder);
	RESULT resultChain={ NULL,};
	RESULT result={ NULL,};
	RESULT matOutput={ NULL,};
	matOutput=(struct RESULT) { NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL };
	allocate_single_output_trend(&resultChain,&opt,&MEM);
	allocate_single_output_trend(&result,&opt,&MEM);
#if M_INTERFACE==1
#define ANS plhs[0]
#else
	SEXP    ANS=NULL;
	...
	int  nprt=0;
	if (nlhs==1){
		ANS=allocate_output_trend(&matOutput,&opt,&nprt);
	}
#if PTHREAD_INOUT==1
	...
	scaleFactorSeason=NULL;
	scaleFactorTrend=MEM.alloc(&MEM,sizeof(float)*(opt.maxKnotNum_Trend+1),0);
	GlobalMEMBuf_1st=Xnewterm;			 
	opt.maxKnotNum_Season=opt.maxKnotNum_Trend;
	GlobalMEMBuf_2nd=Xnewterm+( max(opt.maxKnotNum_Season,opt.maxKnotNum_Trend)+1);
	...
	YINFO	yInfo;
	yInfo.Y=(float *)   MEM.alloc(&MEM,sizeof(float)* opt.Npad,64);
	yInfo.rowsMissing=(uint32_t *)MEM.alloc(&MEM,sizeof(uint32_t)* opt.N,64);
#if PTHREAD_INOUT==1
	...
	T_propProb.birth=(uint8_t)(opt.resamplingTrendOrderProb *  0.33 * 255);
	T_propProb.death=(uint8_t)(opt.resamplingTrendOrderProb * (0.33+0.165) * 255);
	T_propProb.merge=(uint8_t)(opt.resamplingTrendOrderProb * (0.33+0.33) * 255);
	T_propProb.move=(uint8_t)(opt.resamplingTrendOrderProb * 1.0 * 255);
#if defined(MSVC_COMPILER)	
...
#include <mach/mach_time.h>
		...
		if (opt.inputFromDisk)
		{
			...
			rI32 readBytes=fread(GlobalMEMBuf_1st,sizeof(float),opt.N,file.infp);
		...
		zero_out_result_output_trend(&opt,&result);
		...
		while (chainNumber< opt.chainNumber)
		{
			...
			zero_out_result_output_trend(&opt,&resultChain);
			basis->tKnotNum=0; 
			...
			{
				convert_basis_both_trend(basis);
				GlobalMEMBuf_1st=Xnewterm;
				evaluate_basis_both_trend_fast(basis,opt.N,Xt_mars,Xt_zeroBackup,&yInfo,SEASON_TERMS,TREND_TERMS,&modelPar,GlobalMEMBuf_1st);
				findGoodKnotPositionFromBasis(basis,NULL,goodT,opt.N,opt.minSepDist_Season,opt.minSepDist_Trend);
				goodT_num=0;
				for (rI32 i=N; i >0; i--)
					goodT_num+=(*goodT++);
				goodT -=N;
			...
			while (sample < opt.samples)
			{
				...
				if (true)
				{
					...
					convert_basis_both_trend(basis_prop);
				...
				}
				{
					FLOAT_SHARE.scale=1;
					if (flag==birth)
						FLOAT_SHARE.scale=true ? scaleFactorTrend[basis->tKnotNum] : scaleFactorSeason[basis->sKnotNum];
					else if (flag==death)
						FLOAT_SHARE.scale=true ? 1.0f/scaleFactorTrend[basis->tKnotNum - 1] : 1.0f/scaleFactorSeason[basis->sKnotNum - 1];
				}
				float factor;
				int delta_k1=basis->tKnotNum ;
				delta_k1++;
				int delta_k2=basis_prop->tKnotNum ;
				delta_k2++;
				...
				else
				{
					...
					int j=max(basis->k - basis->k_SN,basis_prop->k - basis_prop->k_SN);
					int j0=min(basis->k - basis->k_SN,basis_prop->k - basis_prop->k_SN);
					factor=1;
				...
				if (delta_lik > 0||*rnd++<fastexp(delta_lik) * FLOAT_SHARE.scale *   4.294967296000000e+09)
				{
					...
					if (K_newTerm !=0)
						r_cblas_scopy(K_newTerm*Npad,Xnewterm,1L,Xt_mars+(k1_new - 1)*Npad,1L);
					if (true)
					{
						if (flag==birth)
						{
...
#include <mach/mach_time.h>
		...
		if (nlhs==1)
		{
			...
			*((double*)matOutput.sig2+baseIdx)=(double)*result.sig2;
			...
			if (opt.computeChangepoints)
			{
				...
				int32_t             trueCptNumber;
				cptNumber=(int32_t)round((double)*result.tN);
				trueCptNumber=find_changepoint((F32PTR)result.tProb,mem,threshold,cptList,cptCIList,opt.N,opt.minSepDist_Trend,cptNumber);
				int32_t  baseIdx=(pixelIndex - 1)*opt.maxKnotNum_Trend;
				for (int i=0; i <trueCptNumber; i++)
				{
					*((double *)matOutput.tcp+baseIdx+i)=(double)(*(int32_t *)(cptList+i)+1);
				}
				for (int i=trueCptNumber; i < opt.maxKnotNum_Trend; i++)
				{
				...
				for (int i=0; i <trueCptNumber; i++)
				{
					*((double *)matOutput.tcpCI+baseIdx+i)=(double)(*(cptCIList+i)+1);
					*((double *)matOutput.tcpCI+baseIdx+opt.maxKnotNum_Trend+i)=(double)(*(cptCIList+trueCptNumber+i)+1);
				}
				for (int i=trueCptNumber; i < opt.maxKnotNum_Trend; i++)
				{
	...
	if (opt.outputToDisk)
	{
		fclose(file.tN);	fclose(file.tNProb);
		fclose(file.tProb);			fclose(file.t);		fclose(file.b);
		if (opt.computeCredible)  fclose(file.tCI);		if (opt.computeCredible)  fclose(file.bCI);
		if (opt.computeSlopeSign) fclose(file.bsign);
