...
void transpose_inplace(rFLTPTR m,int32_t w,int32_t h ) 
{
...
void  fill_1_to_N(rFLTPTR p,int N)
{
	for (rINT i=1; i <=N; i++) *p++=(float)i;
	p -=N;
}
int32_t strcicmp(char const * _restrict a,char const * _restrict b)
{
	for (;; a++,b++) {
		rINT d=((*a)|(uint8_t)32) - ((*b)|(uint8_t)32);
		if (d !=0||!*a)
			return d;
	}
}
float determine_period(rFLTPTR data,int32_t N,float  omissionValue)
{
	rFLTPTR xData=(float *)r_malloc(sizeof(float)*N * 4);
	float delta=1.f/N;
	float curValue=0.f;
	...
	rFLTPTR XtX=(float *)r_malloc(sizeof(float)* 4 * 4);
	rFLTPTR XtY=(float *)r_malloc(sizeof(float)* 4);
	...
	rFLTPTR yFit=(float *)r_malloc(sizeof(float)* N);
	...
	rFLTPTR ans=(float *)r_malloc(sizeof(float)*M);
	...
	memset(isNA,0,M);
	INTPTR index=(int32_t *)r_malloc(sizeof(int32_t)*M);
	size_t  totalLMnum=0;
...
static float confidenceInterval(rFLTPTR half,int32_t n,char leftOrRight);
int32_t find_changepoint1(rFLTPTR prob,rFLTPTR mem,float threshold,INTPTR cpt,rFLTPTR cptCI,int32_t N,int32_t minSepDist,int32_t maxCptNumber)
{
	if (maxCptNumber==0)	{return maxCptNumber;}
	int32_t w=(int32_t) round((minSepDist - 1)/2);
	int32_t w2=w * 2+1;
	r_ippsSet_32f(0,mem,N);			
	INTPTR cpfromSum_Pos=(int32_t*) mem+N;
	FLTPTR cpfromSum_Val=(float *)mem+N * 2;
	INTPTR cpfromProb_Pos=(int32_t *) mem+N * 3;
	FLTPTR cpfromProb_Val=(float *)mem+N * 4;
...
int32_t find_changepoint(rFLTPTR prob,rFLTPTR mem,float threshold,INTPTR cpt,rFLTPTR cptCI,int32_t N,int32_t minSepDist,int32_t maxCptNumber)
{
	if (maxCptNumber==0)	{ return maxCptNumber; }
	...
	float	delta;
	FLTPTR tmpSeg=(float	* )  mem+3 * N;
	INTPTR nullSeg=(int32_t	*) mem+4 * N;
	for (int32_t i=0; i < cptNumber; i++)
	{
...
static INLINE void swapValue(rFLTPTR a,rFLTPTR b)
{
	rFLT t=*a;
	*a=*b;
	*b=t;
}
static INLINE void swapIndex(INTPTR a,INTPTR b)
{
	rINT t=*a;
	*a=*b;
	*b=t;
}
int32_t partitionD(rFLTPTR arr,INTPTR index,int32_t low,int32_t high)
{
	rFLT pivot=arr[high];    
	rINT i=(low - 1);  
	for (rINT j=low; j <=high - 1; j++)
	{
		if (arr[j] > pivot)
		{
			i++;    
...
void quickSortD(rFLTPTR arr,INTPTR index,int32_t low,int32_t high)
{
	if (low < high)
	{
...
int partitionA(rFLTPTR arr,INTPTR index,int32_t low,int32_t high)
{
...
void quickSortA(rFLTPTR arr,INTPTR index,int32_t low,int32_t high)
{
	if (low < high)
	{
...
float sum_log_diag(rFLTPTR p,rINT K)
{
	rFLT x=0;
	for (rINT i=0; i < K; i++)
	{
